...
export async function createNote(input: { personId: number; content: string }): Promise<number> {
  const data = (await call('/api/v1/notes', {
    method: 'POST',
    body: JSON.stringify({ person_id: input.personId, content: input.content }),
  })) as { data?: { id?: number } };
  const id = data.data?.id;
